...
@section('content_header')
    <div class="d-flex justify-content-between align-items-center">
        <h1 class="m-0">Edit Anggota Lembaga</h1>
    ...
    <div class="card">
        <div class="card-header bg-gradient-primary">
            <h5 class="card-title mb-0 text-white">
                <i class="fas fa-edit me-2"></i>Edit Data Anggota Lembaga
            </h5>
        </div>
        
        <div class="card-body">
            <form action="{{ route('anggota-lembaga.update', $item->anggota_id) }}" method="POST">
                @csrf
                @method('PUT')
                    ...
                    <select class="form-control @error('lembaga_id') is-invalid @enderror" 
                        ...
                        @foreach($lembagas as $lembaga)
                            <option value="{{ $lembaga->lembaga_id }}" 
                                {{ old('lembaga_id', $item->lembaga_id) == $lembaga->lembaga_id ? 'selected' : '' }}>
                    ...
                    <select class="form-control @error('warga_id') is-invalid @enderror" 
                        ...
                        @foreach($wargas as $warga)
                            <option value="{{ $warga->warga_id }}" 
                                {{ old('warga_id', $item->warga_id) == $warga->warga_id ? 'selected' : '' }}>
                    ...
                    <div class="col-md-6">
                        <div class="form-group">
                            ...
                            <input type="date" class="form-control @error('tgl_mulai') is-invalid @enderror" 
                                   ...
                                   value="{{ old('tgl_mulai', $item->tgl_mulai ? $item->tgl_mulai->format('Y-m-d') : '') }}" required>
                            @error('tgl_mulai')
                                <small class="text-danger">{{ $message }}</small>
                            @enderror
                        </div>
                    ...
                    <div class="col-md-6">
                        <div class="form-group">
                            ...
                            <input type="date" class="form-control @error('tgl_selesai') is-invalid @enderror" 
                                   ...
                                   value="{{ old('tgl_selesai', $item->tgl_selesai ? $item->tgl_selesai->format('Y-m-d') : '') }}">
