...
@section('content_header')
    <div class="d-flex justify-content-between align-items-center">
        <h1 class="m-0">Daftar RT</h1>
        <a href="{{ route('rt.create') }}" class="btn pd-add-btn">
...
@section('content')
    ...
    <div class="card pd-animated">
        <div class="card-header pd-gradient">
            <div class="d-flex flex-column flex-md-row justify-content-between align-items-md-center">
                <h3 class="card-title mb-2 mb-md-0" style="color: white;">Filter RT</h3>
                <form method="GET" action="{{ route('rt.index') }}" class="w-100 w-md-auto">
                    <div class="row g-2 align-items-end">
                        <div class="col-md-4 mb-2 mb-md-0">
                            <label class="small mb-1 text-white-50">Cari (Nomor RT / Ketua / Keterangan)</label>
                        ...
                        <div class="col-md-3 mb-2 mb-md-0">
                            <label class="small mb-1 text-white-50">RW</label>
                            <select name="rw_id" class="form-select form-select-sm border-0" style="background: rgba(255,255,255,0.9);">
                        ...
                        <div class="col-md-2 d-flex gap-1">
                            ...
                            <a href="{{ route('rt.index') }}" class="btn btn-outline-light btn-sm">
    ...
    <div class="card pd-animated mt-3">
        <div class="card-header pd-gradient d-flex justify-content-between align-items-center">
            <h3 class="card-title m-0" style="color: white;">Daftar RT ({{ $rts->total() }} data)</h3>
        </div>
            ...
            <div class="table-responsive">
                <table class="table table-bordered table-striped table-hover mb-0">
                    <thead style="background: linear-gradient(135deg, #49bac4ff 0%, #20aa9cff 100%);">
                        <tr>
                            <th width="10%" style="color: white; border: none; text-align: center;">RW</th>
                            <th width="10%" style="color: white; border: none; text-align: center;">Nomor RT</th>
                            <th width="25%" style="color: white; border: none;">Ketua RT</th>
                            <th width="35%" style="color: white; border: none;">Keterangan</th>
                        ...
                        @forelse($rts as $item)
                            <tr>
                                <td class="text-center">
                                    <span class="badge badge-pd-primary">RW {{ $item->rw->nomor_rw }}</span>
                                </td>
                                <td class="text-center fw-bold">{{ $item->nomor_rt }}</td>
                                <td>
                                    @if($item->ketuaRt)
                                        <strong>{{ $item->ketuaRt->nama }}</strong>
                                    @else
                                ...
                                <td class="text-center">
                                    <div class="action-buttons">
                                        <a href="{{ route('rt.show', $item->rt_id) }}" 
                                        ...
                                        <button type="button" class="btn btn-danger btn-sm delete-btn" 
                                                title="Hapus"
                                                data-id="{{ $item->rt_id }}"
                                                data-name="RT {{ $item->nomor_rt }} - RW {{ $item->rw->nomor_rw }}">
                            ...
                            <tr>
                                <td colspan="5" class="text-center text-muted py-5">
                                    <div class="empty-state-pd">
                                        ...
                                        <h5 class="mb-2">Tidak ada data RT</h5>
                                        <p class="text-muted mb-0">
                                            @if(request()->has('q') || request()->has('rw_id') || request()->has('has_ketua'))
                                                Tidak ada RT yang sesuai dengan filter pencarian
                                            @else
                                                Mulai dengan menambahkan data RT baru
                                            @endif
                                        </p>
                                    </div>
                                </td>
            ...
            <div class="d-flex justify-content-between align-items-center flex-column flex-md-row gap-3">
                <div class="data-info-pd">
                    ...
                    <span class="text-muted">
                        Menampilkan <strong>{{ $rts->firstItem() ?? 0 }}</strong> - 
                        <strong>{{ $rts->lastItem() ?? 0 }}</strong> dari 
                        <strong>{{ $rts->total() }}</strong> data
                    </span>
                </div>
                ...
                <div class="pagination-container">
                    ...
                    <nav>
                        <ul class="pagination mb-0">
                            ...
                            @else
                                <li class="page-item">
                                    <a class="page-link" href="{{ $rts->previousPageUrl() }}" style="
                            ...
                            @if($start > 1)
                                <li class="page-item">
                                    <a class="page-link" href="{{ $rts->url(1) }}" style="
                                        border: 2px solid #e2e8f0;
                                        border-radius: 10px;
                                        margin: 0 4px;
                                        padding: 8px 16px;
                            ...
                            @for ($page = $start; $page <= $end; $page++)
                                ...
                                @else
                                    <li class="page-item">
                                        <a class="page-link" href="{{ $rts->url($page) }}" style="
                                            border: 2px solid #e2e8f0;
                                            border-radius: 10px;
                                            margin: 0 4px;
                                            padding: 8px 16px;
                            ...
                            @if($end < $last)
                                ...
                                @endif
                                <li class="page-item">
                                    <a class="page-link" href="{{ $rts->url($last) }}" style="
                                        border: 2px solid #e2e8f0;
                                        border-radius: 10px;
                                        margin: 0 4px;
                                        padding: 8px 16px;
                            ...
                            @if ($rts->hasMorePages())
                                <li class="page-item">
                                    <a class="page-link" href="{{ $rts->nextPageUrl() }}" style="
                                        border: 2px solid #8b5cf6;
                                        border-radius: 10px;
                                        margin: 0 4px;
                                        padding: 8px 16px;
        ...
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <p>Apakah Anda yakin ingin menghapus RT:</p>
...
<script>
    $(document).ready(function() {
        ...
        $('.delete-btn').on('click', function() {
            ...
            const formAction = "{{ route('rt.destroy', ':id') }}".replace(':id', id);
